...
def sakse_tekst(node: dict) -> str:
    ...
    `what`/`how`/`cond`, and so does the text twin. The renderer is besides a
    READ-ONLY copy of the skill's assets, so it cannot be extended from here. The
    layer must therefore be expressed in a field the views ACTUALLY read — `how` is
    the right one: the regime and the measurement chain are how the node is built.
