...
Checks:
  ...
  [C14]  external_research_watch.json carries no forbidden "confirms EFC" /
...
# --- C14: which fields of the watchlist are subject to the discipline -------
#
...
def check_c14_watch_language(issues):
    ...
    if not os.path.exists(WATCH_JSON):
        issues.append(Issue("C14", "public", "external_research_watch.json missing"))
        return
    try:
        with open(WATCH_JSON, encoding="utf-8") as fh:
            doc = json.load(fh)
    except Exception as e:
        issues.append(Issue("C14", "external_research_watch.json", f"unparseable: {e}"))
        return
    items = doc.get("items") if isinstance(doc, dict) else None
    if not isinstance(items, list):
        issues.append(Issue("C14", "external_research_watch.json",
        ...
        for field in WATCH_TEXT_FIELDS:
            ...
            for phrase in FORBIDDEN_PHRASES:
                if phrase.lower() in low:
                    issues.append(Issue(
                        "C14", f"{key}.{field}", f"forbidden phrase: {phrase!r}"))
...
def main():
    ...
    check_c14_watch_language(issues)
