...
def _findings() -> list[dict]:
    ...
    return [
        {
            "id": "F1",
            "title": "31942821 metadata drift: ansatz presented as derived, plus the sqrt transcription class (be_occupation, x_of_rho, entropy_marginal, KC5)",
            "location": "docs/papers/efc/Derivation_of_the_Entropy_Production/index.json (metadata only; PDF + code are unambiguous)",
            "claim": (
                "index.json description, main_finding and the derived-result equation slot "
                "(core_equations.gamma_result) all presented the phenomenological ansatz "
                "rho/(rho+rho_crit) as the 'derived' result, while key_result, the code "
                "(gamma_total = rho^1.5/(1+rho), gamma_phenomenological = rho/(1+rho)) and PDF "
                "Section 5-6 (Eq. 37-38) state the derived form is Scenario B "
                "rho^(3/2)/(rho+rho_crit), with the ansatz an approximation (~20%). The same "
                "file carried FOUR instances of one sqrt transcription: be_occupation (exponent "
                "g/a0 instead of sqrt(g/a0)), x_of_rho (middle term g/a0), entropy_marginal "
                "(x = g/a0) and the KC5 prose (e^{g/a0}) — PDF Eq. 1/4/11 and the code all use "
                "sqrt(g/a0); the earlier audit named only be_occupation and x_of_rho, and it did "
                "not flag the derived-result slot (gamma_result) at all. The same drifted "
                "sentence is mirrored in README.md Overview and metadata.json abstract. All of "
                "these are metadata-only deviations from the normative PDF + code; the "
                "provenance/corrections register (doi_metadata_correction_proposals.json) "
                "expands F1 into eight per-field corrections and reports each one's status."
            ),
            "status": "open",
            "next": (
                "APPLIED (metadata-only) under card t_3a2d983f: eight corrections, all "
                "status=applied and machine-verified live against index.json, plus the two "
                "mirror sentences in README.md and metadata.json. Staged for review; closes on "
                "merge (human word). Remaining, author-gated: (a) the PUBLISHED PDF's abstract "
                "and Section 1.1 still present the saturating ansatz as the derivation's "
                "outcome — correct that only as a new published version; (b) sealed prediction "
                "P1's property list (linear at low rho, saturating at high rho) is the internal "
                "tension already registered under F9 and form_status_register Gamma_B, "
                "deliberately untouched here because rewriting a sealed prediction is a claim "
                "change, not a transcription; (c) the atlas/engine B+ node text (form split, "
                "F9). The findings vocabulary has no closed state, so how F1 is retired is "
                "itself a decision for the author."
            ),
        },
        {
            "id": "F2",
