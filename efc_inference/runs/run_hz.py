"""
CLI Runner: H(z) Cosmic Chronometer Inference

Usage:
    python -m efc_inference.runs.run_hz --config config_hz.yaml
    python -m efc_inference.runs.run_hz --data efc_inference/data/hubble/hz_starter.csv
    python -m efc_inference.runs.run_hz --dry-run --data efc_inference/data/hubble/hz_smoketest.csv
"""
...
logger = logging.getLogger("efc_inference.hz")


def build_parameters() -> EFCParameterVector:
    """Build parameters for H(z) inference."""
...
def build_meta() -> MetaController:
    ...
    ew.set_level("H0", EpistemicLevel.L3)
    ew.set_level("Omega_m", EpistemicLevel.L2)
    ew.set_level("alpha_cosmo", EpistemicLevel.L0)
...
def run(args):
    """Main H(z) inference run."""
    config = {}
    if args.config:
        config = load_config(args.config)

    data_path = args.data or config.get("data_path",
                                         "efc_inference/data/hubble/hz_smoketest.csv")
...
def main():
    parser = argparse.ArgumentParser(
        description="EFC H(z) Cosmic Chronometer Inference"
    )
