...
class EvolusjonEngine(EFCEngine):
    """Beregnbar populasjonsgenetikk; modellerer ikke artsdannelse eller makroevolusjon."""
    ...
    def regime_node(self, params):
        return {"id":"homo.evolusjon","synlighet":self.SYNLIGHET,"perspektiv":"akademia","stipulasjoner":{"stipulert_av_oss":True},"epistemikk":{"sannhetsstatus":"stottet","evidensstatus":"replikert","konsensusstatus":"institusjonell","sosial_mekanisme":"populasjonsgenetikk","konsensus_er_ikke_sannhet":True},"regime":{"name":"Evolusjon","validity":"allelfrekvens p i [0,1]; likevekt s=0, seleksjon s!=0, drift når |s| neglisjeres mot populasjonsstørrelsen, fiksert p=0/1; NaN utenfor","law_form":"Hardy-Weinberg; p' = p(1+s)/(p(1+s)+(1-p)); t_fix via logistisk seleksjon eller 4N nøytral forventning"},"phase":"regime_engine","measure":{"target":"allelfrekvens og genotypefrekvens","measurer":"populasjonsgenetisk beregning","instrument":"EvolusjonEngine","proxy_chain":["p -> Hardy-Weinberg","fitness -> s -> p'","p,s,N -> fiksasjonstid"],"placement":"generasjoner","compression":"populasjon -> p"},"episenter":"allelfrekvensen","buffer":{"role":"populasjonsstørrelse og drift"},"ontology":{"assumes":["diploid Hardy-Weinberg-populasjon","konstant seleksjon i en generasjon"],"source":"standard population genetics (Hardy-Weinberg; Wright-Fisher)"},"observer":{"bandwidth":"p, s, N","awareness":"instrument_window","er_del_av_systemet":True},"emergence":{"loop":"reproduksjon -> frekvensendring -> neste generasjon","properties":["likevekt","seleksjon","drift","fiksasjon"]},"fractal":{"pattern":"frekvensendring over generasjoner"},"coupling":{"local":"en allel","global":"homo.fluxus som emergent rammeverkshypotese"},"maale_paradigme":{"koordinater":["fraksjon","tid"],"enheter":"andel, generasjoner","status":"avledet"},"nivaa":{"indeks":0,"forelder":None,"tidsskala":"generasjoner","lengdeskala":"populasjon"}}
