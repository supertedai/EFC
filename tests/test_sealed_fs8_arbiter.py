...
def test_manglende_felt_venter():
    for field in ("fsigma8", "sigma", "z_eff", "tracer", "kilde"):
        ...
        assert "invalid" in verdict["årsak"]
...
def test_rapporten_maaler_mu_kanalen_arlig():
    ...
    assert "reproducible" in report["ærlighet"]
    assert "NOT" in report["ærlighet"]  # does not prove sealed mu_0
    assert "does NOT have the μ channel" in report["ærlighet"]  # injected variant reported
