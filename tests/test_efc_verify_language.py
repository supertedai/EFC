"""Unit tests for the C14 language-discipline check in efc_verify.py.

C14 is the JSON sibling of C3: it enforces AGENTS.md's language discipline
...
The tests drive check_c14_watch_language against temporary watchlist files by
...
def _run_c14(items):
    """Run C14 over a throwaway watchlist and return its issues."""
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mock.patch.object(verify, "WATCH_JSON", path):
            verify.check_c14_watch_language(issues)
        return issues
...
class ForbiddenPhraseTests(unittest.TestCase):
    def test_validates_efc_in_relevance_is_an_error(self):
        ...
        self.assertEqual(errs[0].code, "C14")
...
class MalformedInputTests(unittest.TestCase):
    ...
    def test_missing_and_non_dict_items_are_tolerated(self):
        issues = _run_c14([{"key": "no-free-text-fields"}, "junk", None])
        self.assertEqual(issues, [])
