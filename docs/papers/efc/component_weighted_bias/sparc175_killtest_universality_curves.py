...
def load_sparc_data(filepath: str) -> Dict[str, Dict]:
    ...
    # Tabular SPARC format — columns: name, dist, r, V_obs, e_V, V_gas, V_disk, V_bul, ...
    with open(filepath, 'r') as f:
        for line in f:
            line = line.strip()
            ...
            # Optional baryonic component velocities (columns 5,6,7); default 0 if absent
            try:
                v_gas = float(parts[5]) if len(parts) > 5 else 0.0
                v_disk = float(parts[6]) if len(parts) > 6 else 0.0
                v_bul = float(parts[7]) if len(parts) > 7 else 0.0
            except (ValueError, IndexError):
                v_gas = v_disk = v_bul = 0.0
    ...
    for name in galaxies:
        for key in list(galaxies[name].keys()):
...
def run_universality_analysis(data_dir: str, output_dir: str = ".",
                              save_curves: bool = False,
                              curves_filename: str = "sparc175_model_curves.json") -> Dict:
    print("=" * 70)
    ...
    results = []
    curves_output = []  # per-galaxy datapoint records (for --save-curves)
    ...
    for i, (name, data) in enumerate(sorted(galaxies.items())):
        ...
        v_obs = data["v_obs"]
        v_err = data["v_err"]
        v_gas = data.get("v_gas")
        v_disk = data.get("v_disk")
        v_bul = data.get("v_bul")
        ...
        if save_curves:
            v_efc_at_r = efc_velocity(r,
                                      efc_result["v_flat"],
                                      efc_result["r_turnon"],
                                      efc_result["sharpness"])
            v_nfw_at_r = nfw_velocity(r, nfw_result["V200"], nfw_result["c"])
            curves_output.append({
                "galaxy": name,
                "n_points": int(len(r)),
                "delta_aic": float(delta_aic),  # AIC_NFW - AIC_EFC, positive = EFC wins
                "regime": regime,
                "verdict": verdict,
                "best_fit": {
                    "efc": {
                        "v_flat": float(efc_result["v_flat"]),
                        "r_turnon": float(efc_result["r_turnon"]),
                        "sharpness": float(efc_result["sharpness"]),
                    },
                    "nfw": {
                        "V200": float(nfw_result["V200"]),
                        "c": float(nfw_result["c"]),
                    },
                },
                "datapoints": [
                    {
                        ...
                        "V_obs": float(v_obs[j]),
                        "sigma": float(v_err[j]),
                        "V_EFC": float(v_efc_at_r[j]),
                        "V_NFW": float(v_nfw_at_r[j]),
                        "V_bulge": float(v_bul[j]) if v_bul is not None else 0.0,
                        "V_disk":  float(v_disk[j]) if v_disk is not None else 0.0,
                        "V_gas":   float(v_gas[j]) if v_gas is not None else 0.0,
                    }
    ...
    if save_curves and curves_output:
        curves_path = os.path.join(output_dir, curves_filename)
        curves_metadata = {
            "$schema": "kill_test_v6_universality_curves",
            "metadata": {
                "title": "SPARC 175 per-datapoint model curves",
                "description": "V_EFC and V_NFW evaluated at best-fit params, "
                               "with V_obs, sigma, and baryonic components.",
                "delta_aic_convention": "AIC_NFW - AIC_EFC (positive = EFC wins)",
                "produced_by": "sparc175_killtest_universality_curves.py --save-curves",
                "parent_results": "sparc175_killtest_results.json",
                "n_galaxies_with_curves": len(curves_output),
            },
            "galaxies": curves_output,
        }
        with open(curves_path, 'w') as f:
            json.dump(curves_metadata, f, indent=2)
        print(f"[save-curves] wrote {len(curves_output)} galaxies to {curves_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SPARC 175 Kill-Test Universality Analysis (Kill-Test v6 extension)")
    parser.add_argument("data_dir", help="Directory containing SPARC data")
    parser.add_argument("output_dir", nargs="?", default=".",
                        help="Directory to write results (default: .)")
    parser.add_argument("--save-curves", action="store_true",
                        help="Persist per-datapoint V_EFC, V_NFW, V_obs and components for RCMP analysis")
    parser.add_argument("--curves-output", default="sparc175_model_curves.json",
                        help="Filename for per-datapoint curve output")
    ...
    run_universality_analysis(args.data_dir, args.output_dir,
                              save_curves=args.save_curves,
                              curves_filename=args.curves_output)
