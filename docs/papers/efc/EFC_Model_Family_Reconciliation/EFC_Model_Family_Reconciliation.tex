...
\author{Morten Magnusson \\[2pt]
        \small Symbiose Research, Sola, Norway \\
...
\item \textbf{References.} Citations and the canonical side register (\texttt{docs/validation-ledger/}) are the source of truth for claim status. The packages are not cross-linked directly; their \emph{related packages} fields are heterogeneous --- four of the six packages referenced here have empty fields, while 31941465 records \texttt{builds\_on} and \texttt{informs} relations and 31878760 records a companion relation --- so the linkage between them runs through the validation ledger, which remains the authoritative status graph.
\end{itemize}
...
\end{equation}
For any explicitly defined, differentiable model-specific entropy function $S_{i}(\rho)$, $\dot{S}_{i} = (\mathrm{d}S_{i}/\mathrm{d}\rho)\,\dot{\rho}$; a common $\sigma_{s}$ convention across the three forms is not assumed until units and normalizations are fixed.
...
Here $\rho$ and $\rho_{\mathrm{crit}}$ are treated as dimensional densities. $\Gamma_{A}$ and $\Gamma_{C}$ are dimensionless; the declared $\Gamma_{B}$ expression carries a residual dimension $\sqrt{[\rho]}$ and is not used as a common normalized response until a source-supplied prefactor or reference scale is specified. This dimensional inconsistency is present in the source package itself (31942821, \emph{key result}) and is reported here, not smoothed over. The reconciliation likewise does not establish that $\mathrm{d}S/\mathrm{d}\rho$ (the effective response defined above), the source term in $\Box\phi = \Gamma(\rho)$ (the relativistic action, 31876324), and the grid-action $\Gamma$ are the same normalized physical object; this identification is author-gated.
...
The crucial structural statement: these forms do \emph{not} form a linear maturation chain $\Gamma_{A} \to \Gamma_{B} \to \Gamma_{C}$ in which each step supersedes the last on empirical grounds. They form a \emph{shape bifurcation} with one declared-but-undemonstrated relation (B $\to$ B$^{+}$). The B $\to$ B$^{+}$ relation is a declared companion upgrade within the density-of-states package; ``upgrade'' here is ledger status vocabulary, not a claim of physical improvement. No implication B $\Rightarrow$ B$^{+}$ or B$^{+}$ $\Rightarrow$ $\neg$B, improved fit, or dimensional consistency is established, and supersession status remains \emph{not established}. Treating it as established physical selection would overstate the evidence.

\section{Two model-family axes}
\label{sec:axes}

The bifurcation is cleanly described as two presently separate descriptive \emph{model-family} axes, each intended to admit a future discriminator; only the $E$-axis has a current preregistration proposal, while no $\Gamma$-shape discriminator is preregistered. This is a deliberate restatement of the structure to prevent conflation of the two choices. These model-family axes are distinct from the \emph{preregistered test} axes of Section~\ref{sec:discriminators}; the two taxonomies must not be merged.

\textbf{The $E$-exponent axis.} The screening relation couples energy $E$ to the gravitational acceleration $g$ as $E \propto g^{\alpha_{E}}$. Two exponents appear in the corpus: the linear form $\alpha_{E} = 1$ and the square-root form $\alpha_{E} = \tfrac{1}{2}$. The square-root form derives from the gradient-coupled grid action (31941465), which yields $E \propto \sqrt{g}$ from a minimal three-term Lagrangian with a $|\nabla\Phi|(\nabla\xi)^{2}$ coupling.

\textbf{The $\Gamma$-shape axis.} Independently of the exponent, the response function $\Gamma(\rho)$ is realized in the three forms introduced in Section~3 --- $\Gamma_{A}$ (saturating), $\Gamma_{B}$ (power-law), and $\Gamma_{C}$ (square-root saturation) --- whose mutual relations are declared but undemonstrated.

These choices are organized separately here; physical independence is not demonstrated. In particular, the square root appears in two distinct roles that must not be conflated --- as the $E$-axis exponent ($E \propto \sqrt{g}$) and as the $\Gamma$-axis density dependence ($\Gamma \propto \sqrt{\rho/\rho_{\mathrm{crit}}}$). Selecting $\alpha_{E} = \tfrac{1}{2}$ on the $E$-axis does not select the square-root response form on the $\Gamma$-axis. The reconciliation therefore freezes the structure as \emph{two open selections}, not one. Table~\ref{tab:axes} summarizes.
...
\midrule
$E$-exponent axis & BE exponent $\alpha_{E}$ & $\alpha_{E}=1$ (linear), $\alpha_{E}=\tfrac{1}{2}$ ($\sqrt{\phantom{g}}$) & open \\
$\Gamma$-shape axis & response $\Gamma(\rho)$ & $\Gamma_{A}$ (saturating), $\Gamma_{B}$ (power-law), $\Gamma_{C}$ (square-root saturation) & open \\
\bottomrule
\end{tabular}
\caption{The two model-family axes. Neither is selected by an executed test.}
\label{tab:axes}
\end{table}
...
\textbf{Level 3 --- provenance gap.} The metadata fields $\Delta\chi^{2}$ and significance are \emph{null}, and the recorded status is ``no new fit performed.'' The source metadata supports no new fit performed; the claimed $\ge 5\sigma$ exclusion is therefore unexecuted or externally referenced and remains unsubstantiated until an author supplies provenance. This remains \emph{open and author-gated}. The reconciliation does not resolve it, and no downstream status rests on assuming either reading.
...
\textbf{Discriminator 1 --- SPARC/RAR retrodiction (preregistered Axis~1).} The EFC papers record $a_{0} = 1.2 \times 10^{-10}\ \mathrm{m\,s^{-2}}$ as a provisional value recorded in source packages, not an EFC-derived prediction. The preregistration itself remains \emph{proposed} and awaits author freeze; its own record lists $1.2\times10^{-10}$, $6.5\times10^{-10}$, and $a_{0,\mathrm{eff}} = C^{2}a_{0}$ as circulating candidates and states that the test is INVALID until exactly one is author-frozen. The origin of the frozen value must be labelled MOND/SPARC input, EFC output, or nuisance calibration. A retrodiction of the SPARC/RAR relation under the EFC screening form, with $a_{0}$ held fixed, constitutes the discriminating test for the $E$-exponent axis. Locking $a_{0}$ before the fit is the anti-shopping discipline: the arbiter value is frozen, not fitted and then reported as a prediction.

\textbf{Discriminator 2 --- gravitational-wave siren test (preregistered Axis~2).} The relativistic-action package (31876324) contains an expression for the ratio of gravitational-wave to electromagnetic luminosity distances, $d_{L}^{\mathrm{GW}}/d_{L}^{\mathrm{EM}} = \sqrt{F(\phi_{0})/F(\phi_{z})}$ with $F(\phi) = 1 + \alpha\phi$, where $[\alpha\phi] = 1$ and the field normalization remains to be fixed. A future multi-messenger standard-siren sample tests whether $d_{L}^{\mathrm{GW}} \neq d_{L}^{\mathrm{EM}}$. This is a candidate propagation-sector discriminator; $d_{L}^{\mathrm{GW}} \neq d_{L}^{\mathrm{EM}}$ is not EFC-specific and does not by itself distinguish EFC from GR systematics or other modified-gravity friction models. It becomes uniquely EFC-discriminating only when the coupling parameters ($\alpha$, $\phi_{0}$, $\phi(z)$) are numerically frozen. Those parameters are \emph{not} frozen in this reconciliation; freezing them is a separate, author-gated step.
...
\item No selection on the $E$-exponent axis ($\alpha_{E} = 1$ vs.\ $\alpha_{E} = \tfrac{1}{2}$) is made.
